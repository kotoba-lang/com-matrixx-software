(ns matrixx_software.main
  "Kotodama WASM entrypoint for the Matrixx_software clean-room actor (L4) — Clojure port.
  ...
  facts (`matrixx_software.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "matrixx_software")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"matrixx_software.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "matrixx_software-compat" :tier tier :entities entities} 200])
...
(defn start! [] :matrixx_software-compat/ready)
